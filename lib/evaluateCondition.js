export default function evaluateCondition(condition, data, gameState) {
  ...
  if (typeof condition === 'string') {
    ...
    if (conditionEntry) {
      if (Array.isArray(conditionEntry)) {
        return conditionEntry.every(cond => {
          return typeof cond.func === 'function' ? cond.func(targetData, gameState) : this.evaluateDSLCondition(cond.original, targetData, gameState);
        });
    ...
    return condition.every(cond => this.evaluateCondition(cond, targetData, gameState));
  }
  return false;
}