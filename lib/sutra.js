...
class Sutra {
  ...
  use(subSutra, name, insertAt = this.tree.length) {
  ...
  then(actionOrFunction, data = null) {
    ...
    if (typeof actionOrFunction === 'function') {
      ...
      lastNode.then.push({ action: actionOrFunction, data: data });
    }
  ...
  traverseNode(node, data, gameState) {
    if (node.subtree) {
      ...
      if (subSutra) {
        // Pass the subtree's Sutra for evaluation
    ...
    // For non-subtree nodes, use the current Sutra context
    if (node.action) {
      this.executeAction(node.action, data, node, gameState);
    } else {
