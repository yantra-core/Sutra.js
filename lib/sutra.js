...
class Sutra {
  ...
  evaluateCondition(condition, data) {
    ...
    if (typeof condition === 'string' && this.conditions[condition] && typeof this.conditions[condition].original === 'object') {
      logger('Evaluating DSL condition');
      return this.evaluateDSLCondition(this.conditions[condition].original, data);
    }
 
     
    // Handle named function conditions
    if (typeof condition === 'string' && typeof this.conditions[condition] === 'function') {
      logger('Evaluating named function condition', condition, this.conditions[condition]);
      return this.conditions[condition](data);
    }
  

    // Handle direct function conditions
    if (typeof condition === 'function') {
      logger('Evaluating direct function condition');
  ...
  traverseNode(node, data) {
    ...
    if (node.action) {
      this.executeAction(node.action, data); // Execute action
    } else {
