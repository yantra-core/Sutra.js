...
class Sutra {
  ...
  executeAction(action, data, node) {
    ...
    this.emit(action, mergedData, node);
  }
  ...
  removeNode(path) {
    ...
    const nodeToRemove = pathArray[pathArray.length - 1];
    if (Array.isArray(current) && typeof nodeToRemove === 'number') {
      // If current is an array and nodeToRemove is an index, use splice
      if (current.length > nodeToRemove) {
        current.splice(nodeToRemove, 1);
      }
    } else if (current[nodeToRemove] !== undefined) {
      // If it's a regular object property
      delete current[nodeToRemove];
    }
  }
